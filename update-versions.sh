...
url=$(curl -fsSL "https://api.github.com/repos/slskd/slskd/actions/runs?branch=master&exclude_pull_requests=true" | jq -re '[.workflow_runs[] | select(.name == "CI") | select(.conclusion == "success") | .artifacts_url][0]')
